# Title
Core Skills and Focus Areas

# Category
experience

# Problem
Hiring teams often need an explicit skills map rather than scattered mentions across documents.

# My role
Define and communicate a factual set of core competencies based on documented work and responsibilities.

# What I built
AI/ML system design and deployment; software architecture and engineering; AI-based anomaly detection; agile and project management; CI/CD and QA automation; cybersecurity applications; grant writing and R&D strategy.

# Scale and impact
Skills span hands-on engineering and leadership responsibilities in international, cross-functional environments.

# Tech stack
AI/ML, data mining, anomaly detection, system design, Agile (Scrum/Kanban), CI/CD, AWS-based architectures.

# Key decisions and trade-offs
Lists only what is explicitly stated in the source documents; does not add “nice-to-have” skills without evidence.

# Links
- CV: assets/cv/CV_Piotr_Synak.pdf
